...
const toPx = (v: number | string | null | undefined, dimension: number = 1, dpr: number = 1): number => {
  if (v == null) return 0;
  if (typeof v === 'number') return v * dpr;
  const s = String(v).trim();
  if (s.endsWith('%')) {
    const pct = parseFloat(s) / 100;
    return isNaN(pct) ? 0 : pct * dimension;
  }
  const num = parseFloat(s.replace('px', ''));
  return isNaN(num) ? 0 : num * dpr;
};
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    container.addEventListener('pointermove', onPointer as EventListener, { passive: true });
    ...
    return () => {
      ...
      container.removeEventListener('pointermove', onPointer as EventListener);
      ro?.disconnect();
  ...
  useEffect(() => {
    ...
    const dpr = renderer.dpr || Math.min(window.devicePixelRatio || 1, 2);
    const ox = toPx(offset?.x, renderer.gl.drawingBufferWidth, dpr);
    const oy = toPx(offset?.y, renderer.gl.drawingBufferHeight, dpr);
    program.uniforms.uOffset.value = [ox, oy];
